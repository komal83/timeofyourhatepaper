...
def main():
    ...
    resultList = []
    df_train = pd.read_csv("training_new.csv", sep=",", header=None, names=["id","text","label"])
    while fi < len(filelist)-1:
        print("TRAIN: "+filelist[fi])
        print("========================================");
        df_new_train=pd.read_csv(sys.argv[1] + "/" + filelist[fi], sep=",", header=None, names=["id","text","label"])
        frames = [df_train, df_new_train]
